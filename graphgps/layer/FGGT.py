...
class GraphProxyFisher(nn.Module):
    def __init__(self, hidden_dim: int, n_samples: int = 2, eps: float = 1e-6):
        super().__init__()
    ...
    def forward(self, h: Tensor) -> Tensor:
        ...
        def f(inp):
            return self.probe(inp)

        for _ in range(self.n_samples):
            v = torch.randn_like(h)
            _, jv = jvp(f, (h,), (v,), create_graph=False)
            acc.add_(jv.pow(2))

        fisher_diag = acc / float(self.n_samples)
        return fisher_diag + self.eps


class FisherGatedPerformerAttention(PerformerAttention):
    ...
    def __init__(
        ...
        attn_out_bias: bool = True,
        dropout: float = 0.0,
        fisher_hidden: Optional[int] = None,
        fisher_samples: int = 2,
    ):
        super().__init__(
            channels,
            heads,
        ...
        fisher_hidden = fisher_hidden or channels
        self.head_channels = head_channels
        self.heads = heads

        self.fisher = GraphProxyFisher(
            hidden_dim=fisher_hidden,
    ...
    def forward(self, x: Tensor, mask: Optional[Tensor] = None) -> Tensor:
        ...
        # ---- Fisher metric (no gradients) ----
        with torch.no_grad():
            fisher_diag = self.fisher(x)               # [B,N,C]
            fisher_diag = fisher_diag[..., :self.head_channels]
            fisher_diag = fisher_diag.unsqueeze(1)     # [B,1,N,D]

        sqrt_f = torch.sqrt(fisher_diag + 1e-6)        # [B,1,N,D]
...
class FisherGatedMultiheadAttention(nn.Module):
    ...
    def __init__(
        ...
        fisher_hidden = fisher_hidden or embed_dim
        self.fisher = GraphProxyFisher(
            hidden_dim=fisher_hidden,
            n_samples=fisher_samples,
        )
    ...
    def forward(
        ...
        with torch.no_grad():
            fisher_diag = self.fisher(x)                 # [B, N, C]
            fisher_diag = fisher_diag[..., :self.head_dim]
            fisher_diag = fisher_diag.unsqueeze(1)       # [B, 1, N, D]
        ...
        f = fisher_diag  # [B,1,N,D]

        # qᵀ F q  -> [B, H, N]
        q2 = (q.pow(2) * f).sum(dim=-1)

        # kᵀ F k  -> [B, H, N]
        k2 = (k.pow(2) * f).sum(dim=-1)

        # qᵀ F k  -> [B, H, N, N]
        qk = torch.matmul(q * f, k.transpose(-2, -1))

        # Fisher-weighted squared distance
        directional_fisher = q2.unsqueeze(-1) + k2.unsqueeze(-2) - 2.0 * qk
        # shape: [B, H, N, N]

        gate = self.gate_mlp(directional_fisher.unsqueeze(-1)).squeeze(-1)
        gate = torch.sqrt(gate + 1e-6)

        attn_logits = torch.matmul(q, k.transpose(-2, -1)) * self.scale
        attn_logits = attn_logits * gate

        if attn_mask is not None:
            key_mask = attn_mask[:, None, None, :]   # broadcast over heads and queries
